#!/bin/bash
# Submit quantization experiments with TOML config modification
# Usage: ./submit_quantization_jobs.sh
...
for BITS in "${BIT_WIDTHS[@]}"; do
    ...
    # Create command that modifies TOML and runs training
    CMD="cd ~/berlin25-eurosat && \
export PYTHONPATH=/home/team11/berlin25-eurosat && \
sed -i 's/quantization-bits = [0-9]*/quantization-bits = ${BITS}/' pyproject.toml && \
flwr run . ${FEDERATION} && \
...
echo "  ~/berlin25-eurosat/outputs/{bit_width}bit/"
echo ""
